...
Every day at **22:05 (10:05pm) Asia/Dubai**, Resend emails the saved JSON to FROM_EMAIL if the task array is not empty. Tasks remain saved after sending. Deleting all tasks stops the emails.
...
Use an always-running instance for the 22:05 job. Mount a persistent disk at /var/data and set DATA_DIR=/var/data to preserve JSON across deployments and restarts. Run one instance. Without persistent storage, saved tasks can be lost on deployment.
