...
function App() {
  ...
  return <main>
    ...
    <section className="intro"><span className="eyebrow">MAKE TIME FOR WHAT MATTERS</span><h1>Your day,<br /><em>planned.</em></h1><p>Add your tasks and the time you need.<br />Your saved list arrives by email every evening at 10:11pm.</p></section>
    ...
    {!data ? <div className="card"><p>Connecting to your task list...</p><button onClick={load}>Retry connection</button></div> : <div className="layout">
      ...
      <aside><div className="schedule-card"><span className="eyebrow">DAILY DELIVERY</span><div className="time">10:11<span>PM</span></div><p>Every day · Asia/Dubai</p><div className={'status ' + (data.tasks.length && data.configured ? 'active' : '')}><span />{!data.configured ? 'Email configuration needed' : data.tasks.length ? 'Daily email enabled' : 'Waiting for tasks'}</div><hr /><p>{data.tasks.reduce((total, item) => total + item.minutes, 0)} minutes planned</p><p>{data.delivery.lastSentAt ? 'Last accepted: ' + new Date(data.delivery.lastSentAt).toLocaleString() : 'No email sent yet.'}</p>{data.delivery.lastError && <p className="error-text">{data.delivery.lastError}</p>}</div>
        <section className="card json-card"><h2>Saved JSON</h2><pre>{JSON.stringify(data.tasks, null, 2)}</pre></section>
      </aside>
    </div>}
    <footer>Your tasks, delivered daily.<span>10:11pm, Dubai time.</span></footer>
  </main>;
}
